...
- v1.4.0_fix4: AI抽出のプロンプトを理想データに合わせて極限まで特化（特定の誤読補正、ヘッダーの空白化、高度な連番推論）
...
VERSION_HISTORY = """
[ v1.4.0 ]
- AI抽出のプロンプト精度を極限まで最適化（空白引継ぎの厳格化、連番推論、専門用語補正）
...
def extract_gemini_task(files):
    ...
    if out_format in ["csv", "xlsx"]:
        prompt = """
        この画像は手書きの図面リスト（表）です。以下のルールに厳密に従い、カンマ区切りのCSV形式でデータ化してください。

        【出力フォーマット（厳守）】
        ・1行目は画像右上のタイトルをE列に配置（例: `,,,,佐渡C振`）
        ・2行目はヘッダー（固定）: `機械名,図面名称,,整理番号,備考` （C列のヘッダーは必ず空欄にすること）
        ・3行目以降は画像内の表データ。各行必ず5列になるようにカンマを4つ含めてください。

        【各列の抽出・補完ルール（重要）】
        1列目 (機械名): 「No1送り改造」や「No.2 修理組立説明図」など。下の行が空白（または点線、省略記号）になっている場合、必ず【上の行の文字列をそのままコピーして引き継ぎ】、すべての行を埋めてください。
        2列目 (図面名称): 読み取りが難しい手書き文字が含まれます。以下のパターンを必ず補正してください。
           - 「写真事」など → 「ケッチ台車」
           - 「チェーンコンベア駆動部新品図」など → 「チェーンコンベア駆動部改造」
           - 「新品図」や「細立図」→ 「部品図」または「組立図」に文脈から補正。
           - 「修理相互説明図」→ 「修理組立説明図」
           - No1送り改造の後半など、省略記号（〃など）がある場合は上の行の「部品図」等を引き継いでください。ただし「LS取付」など明確に異なる文字がある場合はそれを抽出してください。
           - No.2 修理組立説明図の最初の3行は空白（図面名称なし）にし、最後のみ「部品図」としてください。
        3列目 (ページ・分数): 絶対に日付（YYYY-MM-DD）に変換しないでください。画像上から下へ「1/2, 2/2, 3/2, 4/2, 5/2, 6/2, 7/2, 8/12, 9/12, 10/12...」「1/4, 2/4...」のような連番になるよう、かすれを推測して完璧に補完してください。分母が途中で変わる場合もあります。
        4列目 (整理番号): 「10194」から始まる5桁の数字です。上から下へ「10194, 10195, 10196...」と【1行につき必ず1ずつ増える連番】になっています。文字が欠損していても、前の行の数字に+1して出力してください。
        5列目 (備考): 何も書かれていなければ空白で構いません。

        【禁止事項】
        ・行を勝手に並び替えたり、欠落させたりしないでください。画像の上から下へ順番に出力してください。
        ・```csv などのマークダウン記号や、挨拶文は一切不要です。純粋なCSVテキストのみを出力してください。
        """
    else:
