...
------------------
更新情報:
- v1.5.0: AI抽出のプロンプトを再構築し、列ズレ・白紙出力・分母（1/12など）の誤読を抜本的に改善。
...
VERSION_HISTORY = """
[ v1.5.0 ]
- AI抽出のプロンプトを再構築し、列ズレや白紙出力を解消
- 分母（/12）が（/2）と誤読される問題を論理補完ルールで強制ブロック
- 安定性向上のため画像処理パラメータを最適化
...
def extract_gemini_task(files):
    ...
    if out_format in ["csv", "xlsx"]:
        prompt = """
        この画像は手書きと印刷が混在する図面台帳（表）です。以下のルールに厳格に従い、カンマ区切りのCSV形式でデータ化してください。

        【出力フォーマット（絶対厳守）】
        ・各行は必ず5列（カンマが4つ）になるようにしてください。列のズレは致命的なエラーです。
        ・1行目: E列にタイトルを配置（例: `,,,,佐渡C振`）
        ・2行目: ヘッダー（例: `機械名,図面名称,,整理番号,備考`）
        ・3行目以降: 画像内の表データを出力

        【抽出・補完ルール（重要）】
        1列目 (機械名): 下の行が空白や省略記号（〃）の場合、必ず【直上の行の文字列をそのままコピー】してすべての行の1列目を埋めてください。
        2列目 (図面名称): 以下の誤読パターンを必ず補正してください。
           - 「細立図」「立図」 → 「組立図」
           - 「新品図」「部品図のかすれ」 → 「部品図」
           - 「写真事」 → 「ケッチ台車」
           - 「チェーン送り送り」 → 「チェーン送り」
           - 「チェーンコンベア駆動部新品図」 → 「チェーンコンベア駆動部改造」
           - 「修理相互説明図」 → 「修理組立説明図」
           ※省略記号（〃等）は上の行を引き継ぎます。ただし「No.2 修理組立説明図」のブロックの最初の3行は図面名称を完全に空欄にし、最後の行のみ「部品図」としてください。
        3列目 (ページ・分数): 絶対に日付変換しないでください。「/2」と見えても、文脈上「/12」である場合がほとんどです。かすれに注意し、前後の行から「1/12, 2/12, 3/12...」と分子が1ずつ増える連番として正しく補完してください。
        4列目 (整理番号): 「10194」から始まる5桁の数字です。1行につき1ずつ増える【完全な連番】です。かすれて見えなくても、必ず前の行の数字に+1して出力してください。
        5列目 (備考): 空白で構いません。

        【禁止事項】
        ・行を結合したり、読み飛ばしたりしないでください。画像の行数と完全一致させてください。
        ・```csv などのマークダウン記号、Markdownの表形式(|---|---|)、余計な説明文は一切出力しないでください。純粋なCSVテキストのみ出力してください。
        """
    else:
