...
1. 以下のサイト等からWindows 11用のインストーラーをダウンロードし、インストールします。
...
def extract_gemini_task(files):
    ...
    if out_format in ["csv", "xlsx"]:
        ...
        2. 表の最上部にあるタイトルは、CSVの1行目の右側の列（E列など）に配置してください。
        3. 左側の列の下が空欄になっている場合、文脈を判断し、上の行の値を自動的に引き継いで空白を埋めてください。
        4. 日付は '2026-03-12' のように2026年の形式（YYYY-MM-DD）に自動変換してください。
        5. 余計な挨拶や説明文、マークダウン記号(```csv等)は一切出力せず、純粋なCSVデータのみを出力してください。
        """
    else:
