...
- v1.4.0_fix: AIへ送信する解像度(DPI)を300に向上、図面台帳の行ズレ防止・完全連番推論プロンプトの導入。
...
def extract_gemini_task(files):
    ...
    if out_format in ["csv", "xlsx"]:
        ...
        2列目 (図面名称): 読み取りが難しい手書き文字を以下のルールに従って補正してください。
           ...
           - 「修理相互説明図」 → 「修理組立説明図」
           - 省略記号は直前の図面名ではなく「空白」にするか、文脈に従ってください。後半の「部品図」の連続などは正しく出力してください。
    ...
    for i, f in enumerate(files, 1):
        ...
        try:
            ...
            for page_num in range(total_pages):
                ...
                page = doc[page_num]
                # ★ DPIを150から300に引き上げ、AIがかすれた文字や小さい数字を読み取れるように強化
                pix = page.get_pixmap(dpi=300)
                mode = "RGB" if pix.n == 3 else "L"
                img = Image.frombytes(mode, [pix.width, pix.height], pix.samples).convert("RGB")
