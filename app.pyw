...
- v1.4.0_fix: 3/4の行に部品図がズレる現象を解消するため、4/4の行にのみ部品図を配置する絶対ルールをプロンプトに追記。
...
VERSION_HISTORY = """
[ v1.4.0 ]
- AI抽出のプロンプト精度を極限まで最適化（行ズレの絶対防止、連番推論、専門用語補正）
- 特定の行（4/4の部品図など）のズレを防止するピンポイントルールの追加
- 画像送信時のDPIを300に引き上げ、手書きの視認性を向上
...
def extract_gemini_task(files):
    ...
    if out_format in ["csv", "xlsx"]:
        ...
        2列目 (図面名称): 読み取りが難しい手書き文字を以下のルールに従って補正してください。
           ...
           - 「修理相互説明図」 → 「修理組立説明図」
           - ★重要★「No.2 修理組立説明図」のブロックにおいて、3列目が「4/4」の行のみ図面名称を「部品図」とし、「1/4」「2/4」「3/4」の行の図面名称は絶対に空欄のままにしてください。
           - 省略記号は直前の図面名ではなく「空白」にするか、文脈に従ってください。
        3列目 (ページ・分数): 絶対に日付変換しないでください。画像上から下へ「1/2, 2/2, 3/2, 4/2, 5/2, 6/2, 7/2, 8/12, 9/12...」「1/4, 2/4...」のような分数です。分母が途中で変わる（/2から/12になるなど）ことに注意し、かすれていても前後の行から推測して連番で出力してください。
