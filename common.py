...
VERSION_HISTORY = """
[ v2.10.0 ]
- 【機能追加】ファイル・フォルダの「ドラッグ＆ドロップ」読み込みに対応しました。エクスプローラーから直接ファイルを投げ込むだけで選択が完了します。
- 【UI改善】抽出範囲の選択画面（クロップ画面）を開いた際、PDFが常に画面いっぱいの最適なサイズで表示されるように調整しました。
- 【機能改善】処理中の「ステータス表示」を強化しました。PDFの読み込み中やプレビュー生成中など、現在何を行っているかが一目で分かるようになりました。
- 【機能改善】PDFアナライザー（構造確認）で、複数のファイルを解析する際に「どのファイルが処理中か」の詳細を表示するようにしました。
...
- 【UI改善】ズーム状態を1クリックでリセットできる「全表示」ボタンを追加しました。
- 【機能改善】「テキストのみ抽出（水平線選択）」モードにおいて、文字の中心点ではなく「線が文字に触れているか」で判定する高精度ロジックを導入しました。隣接する行の誤抽出を大幅に削減しています。
